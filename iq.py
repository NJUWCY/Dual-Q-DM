...
def iq_loss(agent, current_Q, current_v, next_v, batch):
    ...
    if args.method.loss == "value_expert":
        ...
        if not (args.method.div == "chi" or args.method.chi):
    ...
    if args.method.regularize:
        ...
        y = (1 - done) * gamma * next_v.detach()
        reward = current_Q - y
        loss_dict['policy_reward'] = reward[~is_expert].mean().item()
        # if args.function=="relu":
        bellman_restrict = args.penalty*(torch.relu(args.left - reward)**2 + torch.relu(reward - args.right)**2).mean()
        # elif args.function=="gelu":
        #     bellman_restrict = args.penalty*(F.gelu(args.left - reward)**2 + F.gelu(reward - args.right)**2).mean()
        # elif args.function=="softplus":
        #     bellman_restrict = args.penalty*(F.softplus(args.left - reward)**2 + F.softplus(reward - args.right)**2).mean()
